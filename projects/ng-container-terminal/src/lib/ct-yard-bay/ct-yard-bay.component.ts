...
export class CtYardBayComponent implements OnInit, OnChanges {
    ...
    if (changes['yardBay']) {
      setTimeout(() => {
        ...
        this.svg = this.host
          ...
          .attr(
            'width',
            (this.yardBay.maxRow + 1) * this.cellSize + 2 * this.padding
          )
          .attr(
            'height',
            (this.yardBay.maxTier + 1) * this.cellSize + 2 * this.padding
          );
        this.renderLayout();
        this.updateYardposInfo();
      }, 0);
  ...
  renderLayout() {
    ...
    this.yardBayTierLabelGroup
      .selectAll('text')
      .data(d3.range(1, this.yardBay.maxTier + 1))
      .enter()
      .append('text')
    ...
    this.yardBayRowLabelGroup
      .selectAll('text')
      .data(d3.range(1, this.yardBay.maxRow + 1))
      .enter()
      .append('text')
      .attr('width', this.cellSize)
      .attr('height', this.cellSize)
  ...
  updateYardposInfo() {
    const cell = this.yardposInfoGroup
      ...
      .data(this.yardBay.yardposInfoArray, (pos: YardposInfo, idx) =>
        JSON.stringify(pos)
      );
    ...
    cell
      ...
      .attr('transform', (yardposInfo: YardposInfo) => {
        const x =
          parseInt(this.yardposParser.getP(yardposInfo.yardpos), 10) *
          this.cellSize;
  ...
  renderYardposInfo(selection: d3.Selection<any, any, any, any>) {
    const g = selection
      ...
      .attr('transform', (posInfo: YardposInfo) => {
        const x =
          parseInt(this.yardposParser.getP(posInfo.yardpos), 10) *
          this.cellSize;
        const y =
          (this.yardBay.maxTier - +this.yardposParser.getC(posInfo.yardpos)) *
          this.cellSize;
    ...
    g.append('path')
      .attr('d', data => {
        // 基础图形是一个框
        let path = `M0 0 L${this.cellSize} 0 L${this.cellSize} ${
          this.cellSize
        } L0 ${this.cellSize} Z`;
        if (data.isLocked) {
          // 箱区封锁要画叉
